...
#include <unistd.h>
#include <string.h>
/* User Define for #include */
#include <math.h>
/* User Define for #include Up to Here */
...
/* User Define for #define*/
#define CPU_BRAM_BASE_ADDRESS 0x40000000
#define CDMA_BRAM_BASE_ADDRESS 0xC0000000
/* User Define for #define Up to Here */
...
int main()
{
    ...
    unsigned long long int SrcArray[BUFFER_BYTESIZE ];
    unsigned long long int DestArray[BUFFER_BYTESIZE ];
    ...
    /*======================================================================================
     STEP 1 : Initialize the source buffer bytes with a pattern  and clear the Destination
     	 	  location
 	 ========================================================================================*/
	unsigned int datasize = sizeof(long long int) * BUFFER_BYTESIZE;
  	for (Index = 0; Index < BUFFER_BYTESIZE; Index++)
  	{
  			SrcArray[Index] = 0x0042000000000000 + Index*3/*Index & 0xFF*/;
  			DestArray[Index] = 0x0;
  	}
  	for (Index = 0; Index < 100; Index++)
  	{
			printf("SrcArray[%d]: %llx, DestArray[%d]: %llx\n", Index, SrcArray[Index], Index, DestArray[Index]);
  	}

	//bram_base = (int*)CPU_BRAM_BASE_ADDRESS;
	//bram_base = malloc(sizeof(int)*BUFFER_BYTESIZE);
  	/*======================================================================================
    ...
    // that was mapped as memory is mapped at the start of a page
     ...
     ========================================================================================*/
    memcpy(mapped_dev_base_1, SrcArray, datasize);
    /*======================================================================================
    ...
    //Reset CDMA
      ...
      *((volatile unsigned long *) (mapped_dev_base + XAXICDMA_SRCADDR_OFFSET)) = (unsigned long)DDR_BASE_ADDRESS;
	  ...
	  //User Define: changed to 0xC0000000
      *((volatile unsigned long *) (mapped_dev_base + XAXICDMA_DSTADDR_OFFSET)) = (unsigned long)CDMA_BRAM_BASE_ADDRESS;
      RegValue = (unsigned long)(datasize);
      ...
      *((volatile unsigned long *) (mapped_dev_base + XAXICDMA_BTT_OFFSET)) = (unsigned long)RegValue;
      ...
      //Set the Source Address
      *((volatile unsigned long *) (mapped_dev_base + XAXICDMA_SRCADDR_OFFSET)) = (unsigned long)CDMA_BRAM_BASE_ADDRESS;
      //Set the Destination Address
	  //User Define: changed to 0xC0000000
    ...
    ========================================================================================*/
      memfd_2 = open("/dev/mem", O_RDWR | O_SYNC);
       ...
       printf("Memory mapped at address %p.\n", mapped_base_2);
        ...
        ========================================================================================*/
        memcpy(DestArray, mapped_dev_base_2, datasize);
        /*======================================================================================
       ...
       for (Index = 0; Index < BUFFER_BYTESIZE; Index++)
       {
	//	   printf("SrcArray[%d]: %d, DestArray[%d]: %d\n", Index, SrcArray[Index], Index, DestArray[Index]);
